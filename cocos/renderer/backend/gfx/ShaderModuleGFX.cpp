...
ShaderModuleGFX::ShaderModuleGFX(ShaderStage stage, std::string_view source)
	: ShaderModule(stage)
{
	switch (stage)
	{
...
ShaderModuleGFX::ShaderModuleGFX(const cc::gfx::ShaderStage& stage)
	: ShaderModule(ShaderStage::VERTEX_AND_FRAGMENT)
{
	_gfxStage = stage;
...
void ShaderModuleGFX::init()
{
	...
	// -> #define VULKAN 1X0
	_glslangShader->setEnvInput(glslang::EShSourceGlsl, envStage, glslang::EShClientVulkan, clientInputSemanticsVersion);
	//-> Vulkan 1.X
	...
	if (!success)
	{
		log("%s: GLSLang Parsing Failed:\n%s\n%s",
			__FUNCTION__, _glslangShader->getInfoLog(), _glslangShader->getInfoDebugLog());
