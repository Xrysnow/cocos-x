...
void Texture2DGFX::updateTextureDescriptor(const TextureDescriptor& descriptor, int index)
{
    ...
    if (_info.format != gfx::Format::UNKNOWN)
    {
        ...
        {
            CC_LOG_ERROR(
                "pixel format %s is not supported (feature=0x%x)",
...
void Texture2DGFX::updateCompressedData(
    uint8_t* data, std::size_t width, std::size_t height, std::size_t dataLen, std::size_t level, int index)
{
    updateCompressedSubData(0, 0, width, height, dataLen, level, data);
}

void Texture2DGFX::updateSubData(
    std::size_t xoffset,
    std::size_t yoffset,
    std::size_t width,
    std::size_t height,
...
void Texture2DGFX::updateCompressedSubData(
    std::size_t xoffset,
    std::size_t yoffset,
    std::size_t width,
    std::size_t height,
...
TextureGFX::TextureGFX(const cc::gfx::TextureInfo& info) : TextureBackend()
{
    _info        = info;
...
void TextureGFX::update(const uint8_t* buffer, const cc::gfx::BufferTextureCopy& region)
{
    update(gfx::BufferDataList{buffer}, gfx::BufferTextureCopyList{region});
}

void TextureGFX::update(const cc::gfx::BufferDataList& buffers, const cc::gfx::BufferTextureCopyList& regions)
{
...
{
    return gfx::formatSize(_texture->getFormat(), region.texExtent.width, region.texExtent.height, 1) *
...
uint32_t TextureGFX::getData(std::vector<uint8_t>& buffer, int32_t x, int32_t y, uint32_t width, uint32_t height)
{
    gfx::BufferTextureCopy region;
...
uint32_t TextureGFX::getData(std::vector<uint8_t>& buffer, const cc::gfx::BufferTextureCopy& region)
{
