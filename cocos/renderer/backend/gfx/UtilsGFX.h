...
public:
    ...
    static std::vector<uint32_t> compileGLSLtoSPIRV(
        cc::gfx::ShaderStageFlagBit stage, const std::string& source, int vkMinorVersion = 2);

    static std::string compileSPIRVtoGLSL(const std::vector<uint32_t>& spirv, uint32_t version, bool isES);
