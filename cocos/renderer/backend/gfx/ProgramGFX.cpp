...
ProgramGFX::ProgramGFX(const cc::gfx::ShaderInfo& shaderInfo)
	: Program("", "")
{
			...
			if (it != map.end())
			{
				...
				if (a.location != target.location)
				{
					log("%s: attribute '%s' location mismatch, expect %d, got %d",
						__FUNCTION__, a.name.c_str(), target.location, a.location);
				}
			}
			else
			{
			...
			if (it != map.end())
			{
				...
				if (b.binding != target.binding || b.set != target.set)
				{
					log("%s: uniform block '%s' mismatch, expect binding=%d set=%d, got binding=%d set=%d",
						__FUNCTION__, b.name.c_str(), target.binding, target.set, b.binding, b.set);
				}
				// compare sequence
				for (size_t j = 0; j < std::max(b.members.size(), target.members.size()); ++j)
				{
					if (j >= b.members.size())
					{
						log("%s: uniform '%s' in block '%s' is not assigned",
							__FUNCTION__, target.members.at(j).name.c_str(), b.name.c_str());
					}
					else if (j >= target.members.size())
					{
						log("%s: uniform '%s' in block '%s' is not in shader",
							__FUNCTION__, b.members.at(j).name.c_str(), b.name.c_str());
					}
					else
					{
						...
						if (u1.name != u2.name || u1.type != u2.type)
						{
							log("%s: uniform name mismatch, expect name='%s' type=%d, got name='%s' type=%d",
								__FUNCTION__, u2.name.c_str(), (int)u2.type, u1.name.c_str(), (int)u1.type);
						}
					}
				}
				...
				for (auto& u : b.members)
				{
					if (targetMemberMap.count(u.name))
					{
						...
						{
							log("%s: uniform name mismatch, expect name='%s' type=%d, got name='%s' type=%d",
								__FUNCTION__, target_u.name.c_str(), (int)target_u.type, u.name.c_str(), (int)u.type);
						}
					}
					else
					{
	...
	if (!shaderInfo.samplerTextures.empty())
	{
		...
		for (auto& s : _info.samplerTextures)
		{
			...
			{
				log("%s: sampler texture '%s' is not assigned",
					__FUNCTION__, s.name.c_str());
			}
		}
		for (auto& s : shaderInfo.samplerTextures)
		{
			...
			if (it != tartgetMap.end())
			{
				...
				{
					log("%s: sampler texture '%s' mismatch, expect binding=%d set=%d type=%d, got binding=%d set=%d type=%d",
						__FUNCTION__, s.name.c_str(), target.binding, target.set, (int)target.type, s.binding, s.set, (int)s.type);
				}
			}
			else
			{
		...
		else
		{
			...
			if (ust.type == gfx::Type::UNKNOWN)
			{
				log("%s: sampler uniform '%s' has invalid type '%s'",
				    __FUNCTION__, name.c_str(), s.getString().c_str());
			}
			if (!s.isCombined())
			{
				log("%s: sampler uniform '%s' is not combined",
		...
		else
		{
			...
			for (auto& u : block.members)
			{
				...
				else
				{
					...
					{
						log("%s '%s': uniform '%s' in block '%s' mismatch, expect count=%d size=%d offset=%d, got count=%d size=%d offset=%d",
							__FUNCTION__, info.name.c_str(), u.name.c_str(), block.name.c_str(),
							uinfo.count, uinfo.size, uinfo.bufferOffset, gotCount, size, ex_size);
					}
					uinfo.count = u.count;
					uinfo.size = size;
					uinfo.bufferOffset = ex_size;
			...
			if (!legacy && ex_size != ex.size)
			{
				log("%s '%s': uniform block '%s' mismatch, expect size=%d, got size=%d",
					__FUNCTION__, info.name.c_str(), block.name.c_str(), ex.size, ex_size);
			}
			ex.size = ex_size;
		}
		++i;
...
ProgramGFX::SpirvCodes ProgramGFX::compileSpirvCodes(glslang::TProgram* tProgram,
	const std::vector<cc::gfx::ShaderStageFlagBit>& stages)
{
	SpirvCodes ret;
	for (auto&& stage : stages)
	{
